...
def get_circ_info(fname):
    ...
    table.add_row(
        str(qc.num_qubits),
        str(len(qc)),
        str(qc.num_nonlocal_gates()),
        str(qc.depth()),
        str(qc.depth(lambda instr: instr.operation.num_qubits > 1)),
    )
    console.print(table)

    # print(qc)
    console.print(f"gate names count: {gate_names_count}")
    console.print(f"gate numbers count: {num_gates_count}")
