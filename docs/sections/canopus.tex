...
\begin{figure}[tbp]
    \centering
    \includegraphics[width=\columnwidth]{figures/sqisw_ecp.pdf}
    \caption{Synthesis coverage for $ \left\{ \SQiSW, \ECP \right\} $ gate set. The trivial points ($ \SQiSW $ and $ \ECP $ themselves) are not shown in this figure. 2Q overage regions corresponds to those require (a) 2 $ \SQiSW $ gates or 2 $ \ECP $ gates; (b) 1 $ \SQiSW $ + 1 $ \ECP $; (c) 3 gates (3 $ \SQiSW $, 3 $ \ECP $, 2 $ \SQiSW $ + 1 $ \ECP $, etc. ) from this gate set for synthesis, respectively.}
    \label{fig:sqisw_ecp_coverage}
\end{figure}
...
\begin{align}
    H  =\,  & w_g\, c_{g}  +  w_d\, \Delta_{\mathrm{depth}} \notag\\
    & + (\Delta_{\mathrm{Avg}\{\mathrm{dist}[i,j]\}_F} + k_E \, \Delta_{\mathrm{Avg}\{\mathrm{dist}[i,j]\}_E}) c_{\mathrm{swap}}
\end{align}
